...
class Triangle(object):
    ...
    @property
    def string(self):
        triang_string = '{:4d}'.format(self._mat) if self._mat else ' '*4
...
class Hexagon(object):
    def __init__(self, *args,**kwargs):
        ...
        self.__upper_half = [Triangle(self.__material,'upper'),
                             Triangle(self.__material,'lower'),
                             Triangle(self.__material,'upper')]  # [M1,M2,M3]
        #
        #                 UPPER HALF
        #                 ----------
        #                / \      / \
        ...
        #             /_______\/_______\
        #
        self.__upper_half = [Triangle(self.__material, 'lower'),
                             Triangle(self.__material, 'upper'),
                             Triangle(self.__material, 'lower')]  # [M1,M2,M3]
        #
        #                  LOWER HALF
        #             -------------------  
        #              \      / \       /
    ...
    def string_upper(self):
        final_string = ' '.join(map(lambda a: a.string, self.__upper_half))
    ...
    def string_lower(self):
        ...
        final_string = ' '.join(map(lambda a: a.string, self.__lower_half))
    ...
    def empty(self):
        if self.__upper_half:
            #            self.__upper_half = None
            self.__upper_half = [Triangle(None,'upper'),
                                 Triangle(None,'lower'),
                                 Triangle(None,'upper')]
        if self.__lower_half:
            #            self.__lower_half = None
            self.__lower_half = [Triangle(None,'lower'),
                                 Triangle(None,'upper'),
                                 Triangle(None,'lower')]
    ...
    def up_material(self):
        Mats = [tr.mat for tr in self.__upper_half] if self.__upper_half is not None else []
        if self.neighbour_30 is not None:
            Mats += self.neighbour_30.low_material()
        return Mats

    def low_material(self):
        Mats = [tr.mat for tr in self.__lower_half] if self.__lower_half is not None else []
        if self.neighbour_330 is not None:
